...
The backend exposes `POST /api/v1/run` for the current UI and `POST /api/v1/agent/run` as an SSE trace stream. Each execution is recorded in the existing `synex_runs` Supabase table; the newest `synex_settings` row supplies the DataHub GMS URL.
