...
async def get_latest_agent_settings() -> dict[str, Any]:
    """Fetch newest agent settings, compatible with both updated_at and created_at timestamps."""
    client = get_supabase_client()
    if client is None:
        return {}
    
    # First attempt: order by updated_at
    try:
        response = await asyncio.to_thread(
            lambda: client.table("synex_settings")
            .select("datahub_url,datahub_pat,llm_provider,llm_model,llm_api_key,updated_at,created_at")
    ...
    # Fallback attempt: order by created_at or fetch first row
    try:
        response = await asyncio.to_thread(
            lambda: client.table("synex_settings")
            .select("datahub_url,datahub_pat,llm_provider,llm_model,llm_api_key,created_at")
...
async def get_run_by_id(run_id: str) -> dict[str, Any] | None:
    """Fetch run record by ID from synex_runs."""
    client = get_supabase_client()
    ...
    try:
        ...
        logger.exception("Could not fetch run %s", run_id)
        return None
