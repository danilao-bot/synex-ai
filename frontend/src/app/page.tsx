...
import { Sparkles, Activity, PanelRightClose, PanelRightOpen, RotateCcw, AlertCircle, Play, Info, ArrowRight, Zap, HelpCircle } from 'lucide-react'
...
export default function WorkspacePage() {
  ...
  const { clearHistory, setPrompt } = useWorkspaceStore()
  ...
  const triggerDemoPrompt = (text: string) => {
    setPrompt(text)
    setDemoStep(2)
  }
  ...
  return (
    <div className="flex flex-col h-full bg-background p-6 overflow-hidden relative">
      {/* Header */}
      <header className="flex items-center justify-between mb-6 shrink-0 z-10">
        <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="w-px h-4 bg-surfaceBorder" />
          <button
          ...
          <button
            ...
            title={isRightCollapsed ? 'Show Results' : 'Hide Results'}
          >
      ...
      <div className="flex-1 flex flex-row min-h-0 overflow-hidden gap-5 relative">

        {/* Center: Chat column */}
        <div className="flex-1 flex flex-col min-w-0 bg-[#080C16] border border-surfaceBorder rounded-2xl overflow-hidden">
          <ChatThread />
          <div className="px-4 py-3 border-t border-surfaceBorder/40 bg-[#080C16] shrink-0">
            <PromptConsole />
          </div>
        </div>
        ...
        {!isRightCollapsed && (
          <aside className="w-96 shrink-0 flex flex-col h-full overflow-hidden">
            <div className="flex-1 bg-[#080C16] border border-surfaceBorder rounded-2xl overflow-hidden px-4 py-4">
              <MetadataInspector />
            </div>
          </aside>
        )}
